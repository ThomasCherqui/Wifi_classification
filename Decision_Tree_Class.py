...
class DecisionTree:
    ...
    def visualize_tree2(self, save_path="decision_tree_simple.png"):
        ...
        def draw(node):
            ...
            if node.label is not None:
                ...
                ell = Ellipse((x, y), width=2.2, height=0.9, color="skyblue", ec="black")
                ax.add_patch(ell)
                ax.text(x, y, f"X[{int(node.attribute)}] ≤ {node.value:.1f}",
    ...
    def visualize_tree3(self, save_path="decision_tree.png",
                        node_w=1.6, node_h=0.9, x_gap=0.8, y_step=1.6,
                        dpi=200):
        """
        ...
        def draw_box(ax, x, y, text, is_leaf=False):
            if is_leaf:
                circ = patches.Circle((x, y), radius=node_w / 2, fc="#90EE90", ec="black", lw=1)
                ax.add_patch(circ)
                ...
                rect = patches.FancyBboxPatch(
                    ...
                    node_w, node_h,
                    boxstyle="round,pad=0.12", fc="#FFD580", ec="black", lw=1)
                ax.add_patch(rect)
        ...
        def draw(node):
            ...
            if node.label is not None:
                ...
                up, down = draw_box(ax, x, y, f"X[{int(node.attribute)}] ≤ {node.value:.2f}", is_leaf=False)
